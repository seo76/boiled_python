...
g_way = pygame.image.load \
      ("C:/Users/LG/Desktop/파이썬 던전/적용이미지/장면/길초안.png")
...
def g_move():
    global gmy_change, gmx_change, gm_x, gm_y, gm_pls, step1
    g_cor()
    #방향키만큼 이동
    if event.type == pygame.KEYDOWN:
        if event.key == pygame.K_DOWN:
...
def dice_sie():
    ...
    if (ms_x > 680 and ms_x < 760)and(ms_y > 226 and ms_y < 248):
        if (event.type == pygame.MOUSEBUTTONDOWN): #dice
            held_down = True
    ...
    if (ms_x > 680 and ms_x < 760)and (ms_y > 621 and ms_y < 641): #exit
        if (event.type == pygame.MOUSEBUTTONDOWN): #dice
            held_down = True
            pygame.quit()
            sys.exit()
...
def dice_run():
    global gm_pls, gm_pls, step1, step2, step3, jopal, d_n2, d_n
