...
def g_move():
    global gmy_change, gmx_change, gm_x, gm_y
    g_cor()
    #방향키만큼 이동
    if event.type == pygame.KEYDOWN:
        if event.key == pygame.K_DOWN:
            gmy_change = gmy_change + 67
            screen.blit(n0d,(gm_x,gm_y))
        elif event.key == pygame.K_UP:
            gmy_change = gmy_change - 67
            screen.blit(n0u,(gm_x,gm_y))
        elif event.key == pygame.K_LEFT:
            gmx_change = gmx_change - 67
...
def dice_sie():
    global ms_x, ms_y, d_lst

    ms_x = pygame.mouse.get_pos()[0]
    ms_y = pygame.mouse.get_pos()[1]
    d_lst = [1, 2, 3, 4, 5, 6, 7, 8]
    
    if (ms_x > 680 and ms_x < 760):
        if (event.type == pygame.MOUSEBUTTONDOWN):
            '''if (ms_y > 226 and ms_y < 248): #dice
