...
def cath_cor(): #성당 내에서 좌표제한
    global bmy_change, bmx_change, bm_x, bm_y, out_x, out_y, shot1, shot2
    ...
    '''if ((((136-bm_x)*(136-bm_x)+(336-bm_y)*(336-bm_y) >= 136*136))and \
       ((136-bm_x)*(136-bm_x)+(336-bm_y)*(336-bm_y) <= 146*146)):
        out_x.append(bm_x)
        if (out_x[0] <= bm_x):
            bm_x = out_x[0]'''
...
def g_move(gm_pls): #게임 내 움직임
    global gmy_change, gmx_change, gm_x, gm_y, gm_cnt, step
    g_cor()
...
def dice_sie():
    global ms_x, ms_y, d_n, d_n2, step1
...
def dice_run():
    global gm_pls, step1, step2, step3, jopal, d_n2, d_n, gm_cnt, step
    ...
    if (step1 == 100):
        screen.blit(dice8,(0,0))
    if (step2 == 100):
        screen.blit(dice1,(0,0))
    if (step3 == 100):
        screen.blit(dice4,(0,0))
